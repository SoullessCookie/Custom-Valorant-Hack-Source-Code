    ...
    public class api
    {
        ...
        public void init()
        {
            ...
            else if (json.message == "invalidver")
            {
                ...
                string new_name = "SkyCheats" + RandomString(4) + ".rar";

                WebClient webClient = new WebClient();
                webClient.DownloadFile(json.download, new_name);

                ProcessStartInfo proc = new ProcessStartInfo();
                proc.UseShellExecute = true;
                proc.WorkingDirectory = Environment.CurrentDirectory;
                proc.FileName = new_name;
        ...
        private void load_user_data(user_data_structure data)
        {
            ...
            config.subscription = user_data.subscriptions[0].subscription; // array of subscriptions (basically multiple user ranks for user with individual expiry dates 
            config.timeleft = user_data.subscriptions[0].timeleft; // array of subscriptions (basically multiple user ranks for user with individual expiry dates 
            config.expiry = user_data.subscriptions[0].expiry; // array of subscriptions (basically multiple user ranks for user with individual expiry dates 

            //MessageBox.Show(Convert.ToString( UnixTimeToDateTime(long.Parse(config.expiry))-DateTime.Now));
            config.expirylefttime = expirydaysleft();
        ...
        public string expirydaysleft()
        {
            ...
            result = "";
             ...
             //int months = Convert.ToInt16(difference.Days/30);
             int days = difference.Days/*-(months * 30)*/;
             int hours = difference.Hours;

            //if (months > 0)
            //    result += " Months " + Convert.ToString(months);
            if (days > 0)
                result += " Days " + Convert.ToString(days);
            if (hours > 0)
                result += " Hours " + Convert.ToString(hours);
